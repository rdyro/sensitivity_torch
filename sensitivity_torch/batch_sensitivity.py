...
_prod = lambda zs: reduce(mul, zs, 1)
    ...
    def Dzk_solve_fn(z, *params, rhs=None, T=False):
        ...
        if T:
            if optimizations.get("FT", None) is None:
                optimizations["FT"] = torch.linalg.lu_factor(t(Dzk.reshape((blen, zlen, zlen))))
            return torch.linalg.lu_solve(*optimizations["FT"], rhs)
        else:
            if optimizations.get("F", None) is None:
                optimizations["F"] = torch.linalg.lu_factor(Dzk.reshape((blen, zlen, zlen)))
...
def _detach_args(*args):
    return tuple(arg.detach() if isinstance(arg, Tensor) else arg for arg in args)
